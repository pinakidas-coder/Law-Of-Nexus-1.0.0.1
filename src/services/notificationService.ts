...
export const sendStatusEmailAPI = async (req: any, res: any) => {
  const { email, clientName, appointment, newStatus } = req.body;
  if (!email || !appointment || !newStatus) {
  ...
  try {
    ...
    console.error(`[Notification Engine] Error dispatching email notification:`, error);
